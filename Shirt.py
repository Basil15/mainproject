...
def run(shirtdata):
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            shirt_x1 = face_x2 - int(face_w) - int(shirtWidth / 2)  # setting shirt centered wrt recognized face
            shirt_x2 = shirt_x1 + shirtWidth + int(face_w) + 5
            shirt_y1 = face_y2 - 2  # some padding between face and upper shirt. Depends on the shirt img
            shirt_y2 = shirt_y1 + shirtHeight + int(1.5 * h) - 5
            ...
            cv2.imshow('Virtual Dressing Room2', shirt)
            ...
            cv2.imshow('Virtual Dressing Room4', mask_inv)
        ...
        cv2.resizeWindow("Virtual Dressing Room", int(img.shape[1] * 1.4), int(img.shape[0] * 1.4))
        cv2.imshow('Virtual Dressing Room', img)
        cv2.imshow('Virtual Dressing Room1', gray)
        key = cv2.waitKey(10)
        ...
        if key & 0xFF == ord('c'):  # save on pressing 'c'
            ...
            while j >= 10:
                ...
                for (x, y, w, h) in faces:
                    ...
                    shirt_x2 = shirt_x1 + shirtWidth + int(face_w) + 5
                    shirt_y1 = face_y2 - 2  # some padding between face and upper shirt. Depends on the shirt img
                    shirt_y2 = shirt_y1 + shirtHeight + int(1.2 * h) - 5
                ...
                cv2.imwrite('Captures/' + str(rand) + '.png', img)
